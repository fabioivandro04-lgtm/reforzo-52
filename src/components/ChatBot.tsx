...
const ChatBot = () => {
  ...
  const quickReplies = [
    ...
    "Can you help improve our operations?",
  ...
  if (!isOpen) {
    return (
      <Button 
        ...
        className="fixed bottom-6 left-6 z-50 bg-primary hover:bg-primary/90 text-primary-foreground rounded-full p-4 shadow-lg hover:shadow-xl transition-all duration-300 border border-primary/20 hover:scale-105"
        size="icon" 
  ...
  return (
    <Card className="fixed bottom-6 left-6 z-50 w-[400px] h-[600px] shadow-xl border border-border/20 bg-background backdrop-blur-md flex flex-col animate-scale-in">
      <CardHeader className="border-b border-border/10 bg-gradient-to-r from-primary/5 to-primary/10 p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-foreground">Business Assistant</h3>
      ...
      <CardContent className="flex flex-col h-full p-0">
          ...
          <div className="space-y-6">
            {messages.map((message, index) => (
              ...
              >
                ...
                {message.role === 'user' && (
                  <div className="w-8 h-8 bg-gradient-to-br from-secondary to-secondary/80 rounded-full flex items-center justify-center flex-shrink-0 mt-1 shadow-sm">
            ...
            {messages.length === 1 && (
              <div className="space-y-4 mt-8 animate-fade-in" style={{ animationDelay: '300ms' }}>
                ...
                <div className="grid gap-3">
                  {quickReplies.map((reply, index) => (
                    <Button 
                      ...
                      variant="outline" 
                      size="sm" 
                      className="justify-start text-left text-sm h-auto py-3 px-4 bg-background/50 hover:bg-muted/80 border-border/30 hover:border-primary/30 transition-all duration-200 hover:shadow-sm"
                    >
                      {reply}
                    </Button>
                  ))}
        ...
        <div className="p-6 border-t border-border/10 bg-background/80 backdrop-blur-sm">
          <div className="flex gap-3 items-end mb-4">
            <div className="flex-1 relative">
              <Input 
                ...
                disabled={isLoading}
                className="bg-muted/30 border-border/30 focus:border-primary/50 focus:ring-primary/20 text-sm h-12 pr-12 rounded-xl resize-none"
              />
            </div>
            <Button 
              onClick={sendMessage}
              ...
              size="icon"
              className="bg-primary hover:bg-primary/90 text-primary-foreground h-12 w-12 flex-shrink-0 rounded-xl shadow-sm hover:shadow-md transition-all duration-200"
            >
          ...
          <Button 
            ...
            variant="link"
            size="sm"
            className="text-xs text-muted-foreground hover:text-primary h-auto p-0 font-normal transition-colors duration-200"
          >
